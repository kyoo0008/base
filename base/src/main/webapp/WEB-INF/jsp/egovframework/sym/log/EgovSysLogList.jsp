...
<script type="text/javascript">
	...
	function fn_egov_select_sysLog(pageNo){
		...
		document.frm.action = "<c:url value='/sym/log/SelectSysLogList.do'/>";
		document.frm.submit();	
	}
	
	function fn_egov_inqire_sysLog(requstId){
		var url = "/sym/log/InqireSysLog.do?requstId="+requstId;
...
<body>
<form name="frm" action ="<c:url value='/sym/log/SelectSysLogList.do'/>" method="post">
<input type="hidden" name="cal_url" value="<c:url value='/sym/cmm/EgovNormalCalPopup.do'/>" />

 <table width="700" cellpadding="8" class="table-search" border="0">
	 <tr>
	  <td class="title_left">
	   <img src="<c:url value='/images/egovframework/sym/log/icon/tit_icon.gif' />" width="16" height="16" hspace="3" align="absmiddle">&nbsp;시스템 로그조회</td>
	  <th>
	   발생일자
	  </th>
	  <td>
	  ...
	  <th>
	   <table border="0" cellspacing="0" cellpadding="0">
	    <tr> 
	      ...
	      <a href="javascript:fn_egov_select_sysLog('1')">조회</a> 
	      </td>
	...
	<table width="700" cellpadding="8" class="table-line">
	 <thead>
	  <tr>
	    ...
	    <th class="title" width="10%" nowrap>처리시간</th>   
	 ...
	 <c:forEach var="result" items="${resultList}" varStatus="status">
	  <tr>
	    ...
	    <a href="javascript:fn_egov_inqire_sysLog('<c:out value="${result.requstId}"/>');" 
