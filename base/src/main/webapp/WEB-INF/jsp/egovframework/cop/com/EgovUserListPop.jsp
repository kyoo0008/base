...
<script type="text/javascript">
	...
	function fn_egov_returnUserInfo(userId, userNm, userId){
		var retVal = userId +"|"+userNm+"|"+userId;
	...
	<table width="100%" cellpadding="8" class="table-line">
	 <thead>
	  <tr>
	    ...
	    <th class="title" width="15%" nowrap>사용자아이디</th>
	    <th class="title" width="15%" nowrap>사용자명</th>
	    <th class="title" width="23%" nowrap>주소</th>
	    <th class="title" width="12%" nowrap>이메일</th>
		  ...
		  <tr>
			...
			<c:if test="${result.useAt == 'Y' || result.deletedAt == 'Y'}">
	    		<input type="button" name="selectUser" value="선택" 
		    			onClick="javascript:fn_egov_returnUserInfo('<c:out value="${result.userId}" />','<c:out value="${result.userNm}" />','<c:out value="${result.userId}" />');"  />
			</c:if>
			</td>	    
		  </tr>
		 </c:forEach>	  
