...
function addComment(commentNo, num, ordrCode, ordrCodeDp) {
	...
	jQuery("#replyComment"+num).html(
			"<div class='comment'>"+
				...
				"<form name='commentVO' action='${pageContext.request.contextPath}${_PREFIX}/insertReplyComment.do' method='post' onsubmit='return fn_egov_insert_commentList(this);'>"+
					...
					"<textarea name='commentCn' rows='0' cols='0' placeholder='<spring:message code="cop.comment"/>을 입력하세요.'></textarea>"+
					"<input type='image' src='${_IMG}/board/btn_reply_ok.gif' alt='<spring:message code="button.create"/>' align='absmiddle'/>"+
				"</form>"+
				"</div>"+
			...
			<div class="reply type<c:out value="${result.ordrCodeDp+1 }"/>">
				...
				<span class="date"><fmt:formatDate value="${result.frstRegisterPnttm}"  pattern="yyyy-MM-dd"/>
					<c:if test="${SE_CODE >= brdMstrVO.registAuthor}">
						<a href="#reply_txt" onclick="return addComment(${result.commentNo}, ${status.count}, '${result.ordrCode }', ${result.ordrCodeDp });"><img src="${_IMG}/board/btn_reply_re.gif" alt="<spring:message code="button.reply"/>" /></a>
					</c:if>
					<c:if test="${not empty USER_INFO.emplyrId and result.frstRegisterId eq USER_INFO.emplyrId}">
						...
						<a href="<c:out value="${delUrl}"/>" onclick="fn_egov_deleteCommentList(this.href);return false;"><img src="${_IMG}/board/btn_delete.gif" alt="<spring:message code="button.delete"/>" /></a>
					</c:if>
				</span>
				<p class="reply_cont">
	...
	<div class="comment">
		<c:choose>		
		<c:when test="${not empty USER_INFO.emplyrId and SE_CODE >= brdMstrVO.registAuthor}">
			<div class="comment_inp">
				<form name="comment" id="comment" method="post" action="<c:url value='${_PREFIX}/insertComment.do'/>" onsubmit="return fn_egov_insert_commentList(this);">
					...
					<textarea name="commentCn" rows="0" cols="0" placeholder="<spring:message code="cop.comment"/>을 입력하세요."></textarea>
					<input type="image" src="${_IMG}/board/btn_reply_ok.gif" alt="<spring:message code="button.create"/>"/>
					<p><spring:message code='cop.comment.msg' /></p>
				</form>
		  	</div>
		</c:when>
		<c:otherwise>
			<div class="comment_inp">
				...
				<img src="${_IMG}/board/btn_reply_ok.gif" alt="<spring:message code="button.create"/>" />
				<p><spring:message code='cop.comment.msg' /></p>
		  	</div>
		</c:otherwise>
		</c:choose>
