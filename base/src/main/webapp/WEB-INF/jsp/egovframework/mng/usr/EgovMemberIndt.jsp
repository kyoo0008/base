...
alert("${message}");
		...
		function fn_egov_ZipSearch() {
			url = "${pageContext.request.contextPath}/search/link/road.jsp"
...
<div id="cntnts">
	...
	<form:form commandName="userManageVO" name="userManageVO" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/mng/usr/EgovUserSelectIndt.do"> 
	...
	<table class="chart2">
		...
		<tr>
			...
			<td><%-- <form:input path="userId" id="userId" cssClass="inp" /> <form:errors path="userId" /> --%>
		...
		<tr>
			...
			<td><form:input  path="password" id="password" cssClass="inp" /> <form:errors path="password" />
			<%-- 
		...
		<tr>
			...
			<td colspan="2">
				...
				<form:input path="adres" maxlength="100" cssClass="inp addr" title="주소입력" size="55px" />
		...
		<tr>
			...
			<td colspan="2"><form:hidden path='brthdy'/>
				...
				<input type="radio" id="slrcldLrrCode" name="slrcldLrrCode" value="01" <c:if test="${userManageVO.slrcldLrrCode ne '02'}"> checked</c:if>/>양력
