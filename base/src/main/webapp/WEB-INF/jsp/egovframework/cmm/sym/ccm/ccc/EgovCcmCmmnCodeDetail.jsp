...
<div id="cntnts">
	...
	<table class="chart2">
		  ...
		  <tr> 
		    ...
		    <td>
				<select name="useAt" disabled="true">
					<option value="Y" <c:if test="${result.useAt eq 'Y'}">selected="selected"</c:if> >Yes</option>
					<option value="N" <c:if test="${result.useAt eq 'N'}">selected="selected"</c:if> >No</option>				
				</select>
		    </td>    
		  </tr>     
		</table>
		...
		<input type="image" src="${pageContext.request.contextPath}/template/common/images/sub/board/btn_drawing.gif" alt="저장" onclick="return fn_egov_save(this.form);"/>
