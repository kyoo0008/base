$(document).ready(function(){	
	$("#side_button").click(function () {
		if(!$(this).data("status")) {
			...
			$(this).css({background:'url(/cait/str/cre/lyt/tmplat/sit/LYTTMP_0000000000004/images/common/close.png) no-repeat'}).animate({top:'20px'},300);
			$(this).data("status","over");
		}else {
			$( "#fixed" ).animate({left:'-74px'},300);
			$(this).css({"background":"url(/cait/str/cre/lyt/tmplat/sit/LYTTMP_0000000000004/images/common/open.png) no-repeat"}).animate({top:'110px'},300);
			$(this).data("status","");
		}
	});
});
