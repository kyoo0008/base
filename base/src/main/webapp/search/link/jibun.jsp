...
<title>::지번검색::</title>
<link type="text/css" href="<%=request.getContextPath()%>/search/link/style.css" rel="stylesheet" />
<script src="<%=request.getContextPath()%>/search/link/common.js"></script>
 ...
 */
   ...
   function changeAreaList(idx, obj){	
		...
		var url = "<%=request.getContextPath()%>/search/link/AjaxRequestXML.jsp?getUrl=http://www.juso.go.kr/getAreaCode.do?" + escape(createParameter());
	...
	function normalSearch(currentPage){
		...
		var keyword = "";
		var cityText='',countyText='',townText='',riText='',orgCode='217',orgNm='세종특별자치시평생교육연구원';
		...
		url = "/search/link/AjaxRequestXML.jsp?getUrl="+escape("http://www.juso.go.kr/link/search.do?extend=false&mode=jibun_search&searchType=location_jibun&topTab=1&engineCtpNm="+encodeURI(cityText)+"&engineSigNm="+encodeURI(countyText)+"&engineEmdNm="+encodeURI(townText)+"&engineLiNm="+encodeURI(riText)+"&engineBdMaSn="+encodeURI(form.bun1.value)+"&engineBdSbSn="+encodeURI(form.bun2.value)+"&engineMtYn="+encodeURI(form.san1.value)+"&currentPage="+currentPage+"&orgCode="+orgCode+"&orgNm="+encodeURI(orgNm));
	...
	function normalSearch2(currentPage){
		...
		var form = document.check;
		form.currentPage.value =currentPage;
		var orgCode='217',orgNm='세종특별자치시평생교육연구원';

		currentPage=parseInt((currentPage/10)+1);
		
		url = "/search/link/AjaxRequestXML.jsp?getUrl="+escape("http://www.juso.go.kr/link/search.do?extend=false&mode=jibun_search&searchType=location_jibun&topTab=1&engineCtpNm="+encodeURI(form.engineCtpNm.value)+"&engineSigNm="+encodeURI(form.engineSigNm.value)+"&engineEmdNm="+encodeURI(form.engineEmdNm.value)+"&engineLiNm="+encodeURI(form.engineLiNm.value)+"&engineBdMaSn="+form.engineBdMaSn.value+"&engineBdSbSn="+form.engineBdSbSn.value+"&engineMtYn="+form.engineMtYn.value+"&currentPage="+currentPage+"&orgCode="+orgCode+"&orgNm="+encodeURI(orgNm));
	...
	*/
	 ...
	 function setOpenerValue(newaddr){
		 var result = newaddr.split(","); 
		 window.opener.document.getElementById("zip").value = result[2];
		 window.opener.document.getElementById("adres").value = result[0];
		 window.opener.document.getElementById("adresDetail").value = document.getElementById("detail").value+" "+result[1];
	 ...
	 function selectSearch(mode) {
		 if (mode == 'road'){
				document.location.href = "<%=request.getContextPath()%>/search/link/road.jsp";
			}else if(mode =='jibun'){
				document.location.href = "<%=request.getContextPath()%>/search/link/jibun.jsp";
			}else if(mode=='sangho'){
				document.location.href = "<%=request.getContextPath()%>/search/link/sangho.jsp";
			}		
	}
