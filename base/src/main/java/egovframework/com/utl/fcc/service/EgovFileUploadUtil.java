...
public class EgovFileUploadUtil extends EgovFormBasedFileUtil {
     ...
     * @throws Exception
     */
    public static List<EgovFormBasedFileVo> uploadStreamFiles(HttpServletRequest request, String where, long maxFileSize) throws Exception {
	List<EgovFormBasedFileVo> list = new ArrayList<EgovFormBasedFileVo>();
        ...
        vo.setContentType("image/" + contentType);
        vo.setServerSubPath(getTodayString());
        vo.setPhysicalName(getPhysicalFileName() + "." + encoding);
                
    	saveFile(request.getInputStream(), new File(where + seperator + vo.getServerSubPath() + seperator + vo.getPhysicalName()));
    	list.add(vo); 
