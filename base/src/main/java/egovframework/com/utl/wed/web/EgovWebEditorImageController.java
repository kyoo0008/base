...
public class EgovWebEditorImageController {
	 ...
	 * 첨부파일 위치 지정
	 */
    //private String uploadDir = Globals.FILE_STORE_PATH;//EgovProperties.getProperty("Globals.fileStorePath");
	 ...
	 * 첨부 최대 파일 크기 지정
	 */
    private long maxFileSize = 1024 * 1024 * 100;   //업로드 최대 사이즈 설정 (100M)
    ...
    String uploadDir = propertiesService.getString("Globals.fileStorePath");
     ...
     * @throws Exception
     */
    @RequestMapping(value="/utl/wed/insertImageForPhotoEditor.do", method=RequestMethod.POST)
    public void insertImageForPhotoEditor(HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
	// Spring multipartResolver 미사용 시 (commons-fileupload 활용)
	//List<EgovFormBasedFileVo> list = EgovFormBasedFileUtil.uploadFiles(request, uploadDir, maxFileSize);
	...
	List<EgovFormBasedFileVo> list = EgovFileUploadUtil.uploadStreamFiles(request, uploadDir, maxFileSize);
